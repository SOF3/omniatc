...
impl Plugin for Plug {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(EguiPrimaryContextPass, render_dock_system.in_set(EguiSystemSets::Dock));
    }
}
...
pub trait TabType {
    ...
    fn prepare_render(&mut self, _contexts: &mut EguiContexts) {}
}
...
(
    ...
    impl Tab {
        fn prepare_render(&mut self, contexts: &mut EguiContexts) {
            match self {
                $(
                    Tab::$variant(t) => t.prepare_render(contexts),
                )*
            }
        }
    }
    ...
    impl<'w, 's> egui_dock::TabViewer for TabViewer<'w, 's> {
        ...
        fn title(&mut self, tab: &mut Tab) -> WidgetText {
            match tab {
                $(
                    Tab::$variant(t) => do_ps_path!(self.ps, $ps_path; |p| t.title(p.p0())) ,
                )*
            }.into()
        }
        ...
        fn ui(&mut self, ui: &mut egui::Ui, tab: &mut Tab) {
            ...
            match tab {
                $(
                    Tab::$variant(t) => do_ps_path!(self.ps, $ps_path; |p| t.ui(p.p1(), ui, order)),
                )*
            }
        }
        ...
        fn on_close(&mut self, tab: &mut Tab) -> OnCloseResponse {
            match tab {
                $(
                    Tab::$variant(t) => do_ps_path!(self.ps, $ps_path; |p| t.on_close(p.p2())),
                )*
            }
        }
    }
...
fn render_dock_system(
    ...
    for (_, tab) in state.iter_all_tabs_mut() {
        tab.prepare_render(&mut contexts);
    }
...
pub struct ReplaceTab<R: Fn(&mut Tab) -> bool>(pub R);
