...
const DISTANCE_EPSILON: Distance<f32> = Distance::from_nm(0.01);

#[test]
fn test_compute_curve_parts() {
    let center = Position::from_origin_nm(100., 100.);

    let start = center + Distance::vec2_from_nm(Vec2::new(1., 0.));
    let end = center + Distance::vec2_from_nm(Vec2::new(1., 1.));
