...
impl Plugin for Plug {
    fn build(&self, app: &mut App) {
        app.add_systems(app::Update, completion_system.in_set(score::StatisticsUpdater));
    }
}
...
fn completion_system(
    object_query: Query<(CompletionObjectQuery, &mut Destination)>,
    params: CompletionParams,
    mut commands: Commands,
    mut statistics: ResMut<score::Statistics>,
) {
    let mut arrivals = 0;
    let mut departures = 0;
    for (object, mut dest) in object_query {
    ...
    statistics.num_arrivals += arrivals;
    statistics.num_departures += departures;
}
