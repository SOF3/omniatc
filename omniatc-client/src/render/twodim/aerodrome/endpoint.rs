...
impl RegenerateParam<'_, '_> {
    fn regenerate(&mut self, endpoint_entity: Entity) {
        ...
        let all_curve_parts = adjacency.iter().array_combinations().filter_map(|segment_pair| {
            ...
            let [Some(radial1), Some(radial2)] = segment_pair.map(|&segment_entity| {
                let segment = try_log!(
                ...
                let radial = (other_position - intersect).normalize_to_magnitude(DISTANCE_UNIT);
                Some(radial)
            }) else { return None };
            ...
            if dot / DISTANCE_UNIT.magnitude_squared().0 < -0.99 {
                ...
                Some(Either::Left(iter::once((
                    intersect + radial1 * (self.conf.intersection_size / DISTANCE_UNIT),
                    intersect + radial2 * (self.conf.intersection_size / DISTANCE_UNIT),
                ))))
            } else {
                ...
                let turn_center_radial = (radial1 + radial2).normalize_to_magnitude(DISTANCE_UNIT * (2. / (1. + dot)).sqrt());
                ...
                let num_curves_float = (turn_dir / self.conf.arc_interval).abs().ceil();
                ...
                let curve_points = (0..=num_curves).map(move |curve_number| {
                    ...
                    let mut heading = heading1 + turn_dir / num_curves_float * curve_number_float;
                    if heading2.is_between(heading1, heading) {
                        heading = heading2;
                    }
                    ...
                    intersect + pos * (conf.intersection_size / DISTANCE_UNIT)
                });
                Some(Either::Right(curve_points.tuple_windows::<(_, _)>()))
            }
        }).flatten();
