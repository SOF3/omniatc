...
use crate::level::{SystemSets, ground, nav, object, plane, route, taxi, wake};
...
fn spawn_system(mut params: ParamSet<(Trigger, Spawner)>, mut rng: Local<Option<SmallRng>>) {
    let rng = rng.get_or_insert_with(|| SmallRng::from_rng(&mut rand::rng()));
    if params.p0().need_more() {
        params.p1().spawn_once(rng);
    }
}
...
impl Spawner<'_, '_> {
    fn spawn_once(&mut self, rng: &mut impl rand::Rng) {
        ...
        let Some(object_type) = self.object_type_query.log_get(object_type_id) else { return };
        ...
        let Some(resolved_location) = self.resolve_location(location, object_type, rng) else {
            return;
        };
        ...
        let Some(preset) = self.preset_query.log_get(route.preset) else { return };
    ...
    fn resolve_location(
        ...
        match spawn_location {
            Location::Aprons(aprons) => {
                ...
                todo!("find position and heading for {apron:?}")
            }
            Location::Runway(segments) => {
                ...
                todo!("find position and heading for segment")
            }
    ...
    fn is_apron_available(&self, apron: Entity) -> bool {
        // TODO implement
        true
    }
    ...
    fn is_taxiway_available(&self, runway: Entity, half_length: Length<f32>) -> bool {
        // TODO implement
        true
    }
}
