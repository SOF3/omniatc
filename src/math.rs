...
pub fn line_circle_intersect(
    ...
    let a = line_dir.x.powi(2) + line_dir.y.powi(2);
    let b = 2. * (line_dir.x * center_to_start.x + line_dir.y * center_to_start.y);
    let c = center_to_start.x.powi(2) + center_to_start.y.powi(2) - radius_sq;
